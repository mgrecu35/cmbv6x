...
r1L=[]
if iread==1:
    dataL=pickle.load(open('se_DPR_Data_1000_2000.pklz','rb'))
    for d1 in dataL:
        zKu,zKa,bst,bcf,bzd,binBB,lon,lat,\
            reliabFlag,piaSRT,pType,sfcType,dprsfcRate,bsfc=d1
        #print(reliabFlag)
        ...
        if bzd<bcf+40:
            ...
            dn1d,dm1d,rrate1d,zkuc,zkasim,\
                ...
                piaka,dt1,dt2 = cmb.prof1d(bst,bzd,bcf,bsfc,\
                                           ...
                                           -1)
            if reliabFlag==-1:
                r1L.append([rrate1d[bcf],dprsfcRate,piaka-piaku,dsrtPIA])
    stop
else:
    dataL=[]
    
for (ifile,f) in zip(range(0,len(fstKu)),sorted(fstKu)[:]):
    f=f.replace("Ku","DPR")
    fh=Dataset(f)
    ...
    print(ifile,f)
    iplot=1
    a=nonzero(abs(lat[:,12:37])<60)
    b=nonzero(pType[:,12:37][a]==1)
    c=nonzero(binBB[:,12:37][a][b]<=0)
    for i1,j1 in zip(a[0][b][c],a[1][b][c]):
        #print(h0[i1,j1+12])
        if h0[i1,j1+12]>1000 and h0[i1,j1+12]<2000:
            dataL.append([zKu[i1,j1+12,:],zKa[i1,j1,:],bst[i1,j1+12],bcf[i1,j1+12],\
                          bzd[i1,j1+12],binBB[i1,j1+12],lon[i1,j1+12],lat[i1,j1+12],\
                          reliabFlag[i1,j1+12],piaSRT[i1,j1+12],pType[i1,j1+12],sfcType[i1,j1+12],\
                          dprsfcRate[i1,j1+12],bsfc[i1,j1+12]])
    print('nprofs=',len(dataL))
    continue
    ny=30
    if iplot==1:
        ...
        for i1 in range(0,1):
            ...
            plt.pcolormesh(arange(i1*n3,i1*n3+n3),arange(176)*0.125,\
                           zKu[i1*n3:i1*n3+n3,ny,::-1].T,vmin=0,vmax=45,cmap="jet")
            plt.plot(arange(i1*n3,i1*n3+n3),h0[i1*n3:i1*n3+n3,ny]/1000.)
            plt.ylim(0,12.5)
            plt.colorbar()
        plt.subplot(212)
        i1=0
        plt.plot(arange(i1*n3,i1*n3+n3),binBB[0:n3,ny])
        plt.xlim(0,n3)
        #plt.ylim(175,140)
        plt.ylim(0,175)
        plt.colorbar()
        ...
        for i1 in range(0,1):
            ...
            plt.pcolormesh(arange(i1*n3,i1*n3+n3),arange(176)*0.125,\
                           zKa[i1*n3:i1*n3+n3,ny-12,::-1].T,vmin=0,vmax=40,cmap="jet")
            plt.plot(arange(i1*n3,i1*n3+n3),h0[i1*n3:i1*n3+n3,ny])
            plt.ylim(0,12.5)
            plt.colorbar()
            if i1==1:
                plt.xlabel("Scan")
...
pickle.dump(dataL,open('se_DPR_Data_1000_2000.pklz','wb'))
