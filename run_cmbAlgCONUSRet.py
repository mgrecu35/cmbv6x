    ...
    for i1 in range(3*n3):
        ...
        dn1d,dm1d,rrate1d,zkuc,zkasim,\
            epst,piaku,piaka = cmb.prof1d(bst[i1,j1],bzd[i1,j1],bcf[i1,j1],bsfc[i1,j1],\
                                          binBB[i1,j1],binBBT[i1,j1],zKu1,zKa1,pType[i1,j1],dr,eps,imu,dnCoeff,dn)#,[n1d])
        
        
        zKaSim[i1,:]=zkasim
        rrate2d[i1,:]=rrate1d
        r1L.append([rrate1d[bcf[i1,j1]],dprsfcRate[i1,j1],pType[i1,j1]])
...
for i1 in range(1,3):
    ...
    plt.subplot(3,1,2)
    plt.pcolormesh(arange(i1*n3,i1*n3+n3),arange(176)*0.125,\
                   zKaSim[i1*n3:i1*n3+n3,::-1].T,vmin=0,vmax=40,cmap="jet")
    plt.plot(arange(i1*n3,i1*n3+n3),h0[i1*n3:i1*n3+n3,24]/1000.)
    plt.ylim(0,4.5)
    plt.subplot(3,1,3)
    plt.pcolormesh(arange(i1*n3,i1*n3+n3),arange(176)*0.125,\
                   rrate2d[i1*n3:i1*n3+n3,::-1].T,vmin=0,vmax=10,cmap="jet")
    plt.ylim(0,4.5)
